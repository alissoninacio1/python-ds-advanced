...
def pass_gen(site: str):
    ...
    #If your goal is simply to turn a string like " john doe " into "Joh", Python has a built-in .capitalize() method:python
    #Removes spaces, grabs first 3 letters, capitalizes index 0
    #adjusted_name = name.replace(" ", "")[:3].capitalize()

    adjusted_name = site.strip()[:3].capitalize()
    print(f"{adjusted_name}.{formatted}")
